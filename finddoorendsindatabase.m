...
%  linelengths = linelengths
  ...
  myj=1;
  ordermatrix=[];
  if (size(parallellinematrix,1) > 0)
  ...
  while (currmetric > 0) && (currmetric > 0.5*maxmetric) && (myj <= size(parallellinematrix,1)-1)
    ...
    maxymetric=[-2*parallellinematrix(myj,3) -2];
    minymetric=[-2*parallellinematrix(myj,3) -2];
