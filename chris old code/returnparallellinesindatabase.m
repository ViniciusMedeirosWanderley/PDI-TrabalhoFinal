  ...
  for myi = 1:length(lineorder)
    ...
    for myj = 1:length(lineorder)
      if (myj > myi)
        ...
        if ((newminx < majorminx-(majorlength/3)) || (newminx > majorminx+(majorlength/3))) && ...
           ((newtheta > majortheta - 5) || (newtheta < majortheta+5))
          metric = max([majorlength newlength])-(abs(newlength-majorlength))-abs(newminy-majorminy)-abs(newmaxy-majormaxy);
          myk = 1;
